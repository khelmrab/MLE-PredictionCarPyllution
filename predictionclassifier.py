...
def verification_version_predictionmodelRandomForestRegressor(username,password,data):
        ...
        data['ep']=ep
        data['year']=year"""
        r = requests.post( url='http://{address}:{port}/predictionmodelRandomForestClassifier'.format(address=api_address, port=api_port),json=data, auth=HTTPBasicAuth(username, password))
        ...
        request done at "/predictionmodelRandomForestClassifier"
        | username={username}
        | password={password}
        |data={data}
...
verification_version_predictionmodelRandomForestRegressor("alice","wonderland", {"W (mm)": 2456,
  ...
  "ep (KW)": 55,
  "year": 2015})
verification_version_predictionmodelRandomForestRegressor("alice","wonderland", {"W (mm)": 456,
  "At1 (mm)": 441,
  "At2 (mm)": 434,
  "Ft": 3,
  "ec (cm3)": 99,
